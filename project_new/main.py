...
# Receive the number of boxes from the keyboard
numBoxes = input('Enter the number of boxes:')

# Convert string to int, because the return value of the 'input' function is string
...
for i in range(days):
    rand = random.randint(0, numBoxes - 1)
...
for i in range(numBoxes):
    randPathStart.append(i)

# the number of trial
trial = 0

# Flag for check of file existence
bCsvExist = False

boxXLoc = []
boxYLoc = []
bStart = False  # Get started or not
# numBoxes = 4
boxesYCoord = 400
boxesXStartCoord = 50
boxesXSpacing = 100
...
def foundCatAnimation():
    ...
    while currentY >= 300:
        ...
        data = pd.DataFrame([[trial, currentDay]], columns=['trial', 'day'])
...
def restartGame():
    ...
    awaitingReset = False
    bStart = False
    displayButton((105, 105, 105), (211, 211, 211), runBtnX, runBtnY, runBtnWidth, runBtnHeight, "START")
...
def displayTrial(num):
    # rgb(128, 128, 128) is grey, (0, 0, 0) is black, (255, 255, 255) is white
    display = font.render("Trial " + str(num + 1), True, (0, 0, 0))
    # x coordinate is 10 pixels away from the edge
    screen.blit(display, (SCREENHEIGHT - display.get_width() - 10, display.get_height() * num + 10))
...
while running:
    ...
    for event in pygame.event.get():
        ...
        if event.type == pygame.MOUSEBUTTONDOWN:
            ...
            # Determine if 'run' button is clicked, or not
            if runBtnX <= x and x <= runBtnX + runBtnWidth and \
               runBtnY <= y and y <= runBtnY + runBtnHeight:
               bStart = True

            if bStart is False:
                continue

            # Click boxes accorindg to random values generated.
            if not awaitingReset:
                loadBoxImages()
                for trial in range(5):

                    for i in range(days):
                        boxSelection = rand_list[i]
                ...
                if os.path.exists('data.csv'):
                    ...
                    with open('data.csv','r') as csvfile:
                        ...
                        plt.plot(x, y, color = 'g', linestyle = 'dashed',
                                marker = 'o',label = "Cat Data")
                        
                        print(x, y)
                        plt.xticks(rotation = 25)
                        plt.xlabel('Trial')
                        plt.ylabel('Date')
                        plt.title('Cat Finding Report', fontsize = 20)
                        plt.grid()
                        plt.legend()
                        plt.show()
            ''' if not awaitingReset:
